...
async function getAliItem() {
  try {
    ...
    // pagename
    productItem.value.pagename = productItem.value.pagename || getTextFromDom('.spuBase div.title')

    // price
    let price = (() => {
      const priceLabel = Array.from(document.querySelectorAll('span.item-name')).find(
        (el) => el.textContent?.trim() === '发售价格'
      )
    ...
    // ageGroup
    let ageGroup = (() => {
      const ageLabel = Array.from(document.querySelectorAll('span.item-name')).find(
        (el) => el.textContent?.trim() === '适用年龄'
      )
    ...
    // date
    let date = (() => {
      const dateLabel = Array.from(document.querySelectorAll('span.item-name')).find(
        (el) => el.textContent?.trim() === '发售日期'
      )
    ...
    let descElementList = Array.from(document.querySelectorAll('.list .image img'))
